...
func (c *Congress) snapshot(chain consensus.ChainHeaderReader, number uint64, hash common.Hash, parents []*types.Header) (*Snapshot, error) {
	...
	for snap == nil {
		...
		if number == 0 || (number%c.config.Epoch == 0 && (len(headers) > params.FullImmutabilityThreshold || chain.GetHeaderByNumber(number-1) == nil)) {
			...
			if checkpoint != nil {
				...
				log.Info("Quasar Chain Stored checkpoint snapshot to disk", "number", number, "hash", hash)
				break
			}
		}
	...
	if snap.Number%checkpointInterval == 0 && len(headers) > 0 {
		...
		log.Trace("Quasar Chain Stored voting snapshot to disk", "number", snap.Number, "hash", snap.Hash)
	}
	return snap, err
}
