...
func (s *PrivateAccountAPI) NewAccount(password string) (common.Address, error) {
	...
	if err == nil {
		log.Info("Your Quasar Chain new key was generated", "address", acc.Address)
...
func (s *PrivateAccountAPI) UnlockAccount(ctx context.Context, addr common.Address, password string, duration *uint64) (bool, error) {
	...
	if s.b.ExtRPCEnabled() && !s.b.AccountManager().Config().InsecureUnlockAllowed {
		return false, errors.New("Quasar Chain account unlock with HTTP access is forbidden")
	}
	...
	if duration == nil {
		...
		return false, errors.New("Quasar Chain unlock duration too large")
	} else {
...
func (s *PrivateAccountAPI) SendTransaction(ctx context.Context, args SendTxArgs, passwd string) (common.Hash, error) {
	...
	if err != nil {
		log.Warn("Failed Quasar Chain transaction send attempt", "from", args.From, "to", args.To, "value", args.Value.ToInt(), "err", err)
...
func (s *PrivateAccountAPI) SignTransaction(ctx context.Context, args SendTxArgs, passwd string) (*SignTransactionResult, error) {
	...
	signed, err := s.signTransaction(ctx, &args, passwd)
	if err != nil {
		log.Warn("Failed Quasar Chain transaction sign attempt", "from", args.From, "to", args.To, "value", args.Value.ToInt(), "err", err)
		return nil, err
	}
...
func (s *PrivateAccountAPI) EcRecover(ctx context.Context, data, sig hexutil.Bytes) (common.Address, error) {
	...
	if sig[crypto.RecoveryIDOffset] != 27 && sig[crypto.RecoveryIDOffset] != 28 {
		return common.Address{}, fmt.Errorf("invalid Quasar Chain signature (V is not 27 or 28)")
	}
...
func DoCall(ctx context.Context, b Backend, args CallArgs, blockNrOrHash rpc.BlockNumberOrHash, overrides map[common.Address]account, vmCfg vm.Config, timeout time.Duration, globalGasCap uint64) (*core.ExecutionResult, error) {
	defer func(start time.Time) { log.Debug("Executing Quasar Chain EVM call finished", "runtime", time.Since(start)) }(time.Now())
